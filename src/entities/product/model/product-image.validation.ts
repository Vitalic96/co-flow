...
export const productImageValidation = z
  ...
  .transform(
    (file) => (file && file.length > 0 && file instanceof FileList && file.item(0)) || (null as unknown as File),
  )
  .refine(fileSizeValidations(), 'The picture must be a maximum of 10MB.')
  .refine(
    imageDimensionsValidation({ maxWidth: 3840, maxHeight: 2160 }),
    'The picture must be width > 3840 and height > 2160',
  )
